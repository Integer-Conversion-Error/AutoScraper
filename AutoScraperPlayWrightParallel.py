...
def parallel_extract(urls, max_workers=5):
    """
    Parallelizes the extraction of `ngVdpModel` for multiple URLs.
    ...
    Returns:
        list: List of results from the extract_ng_vdp_model function.
    """
    results = []
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_url = {executor.submit(extract_ng_vdp_model, url): url for url in urls}
        ...
        for future in as_completed(future_to_url):
            ...
            try:
                ...
                results.append({"url": url, "result": f"Error: {e}"})
    return results
...
def save_to_csv(data, filename="results.csv"):
    ...
    with open(filename, mode="w", newline="", encoding="utf-8") as file:
        ...
        allresults = parallel_extract(data,max_workers=(len(data))/8)
        print(allresults)
        # for link in data:
